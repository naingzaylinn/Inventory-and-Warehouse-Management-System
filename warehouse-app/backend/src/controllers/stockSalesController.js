const service = require('../services/stockSalesService')
...
exports.create = async (req, res) => {
  try {
    ...
    if (!['Sales', 'Sales Return'].includes(reason)) {
      errors.push({ field: 'reason', reason: 'must be Sales or Sales Return' })
    }
    if (!customer_id) errors.push({ field: 'customer_id', reason: 'must not be blank' })
    ...
    if (line_items) {
      line_items.forEach((item, i) => {
        ...
        if (reason === 'Sales' && (!item.quantity_out || item.quantity_out <= 0)) {
          errors.push({ field: `line_items[${i}].quantity_out`, reason: 'must be > 0 for Sales' })
        }
        if (reason === 'Sales Return' && (!item.quantity_in || item.quantity_in <= 0)) {
          errors.push({ field: `line_items[${i}].quantity_in`, reason: 'must be > 0 for Sales Return' })
        }
